...
def load_model(ebd_path_dict,seq_name_path,train_seqs,task_name):
    rep_list = []
...
def load_model_25000(seed,ebd_path_dict,train_name_path,n_train_seqs,task_name):
    rep_list = []
    ebd_path = ebd_path_dict[task_name]
    # with open(seq_name_path) as f:
...
def select_train_seq(task_name,use_25000_train_seqs,real_value_path,train_name_path,seed,n_chains,n_train_seqs,ebd_path_dict,seq_name_path,
TRAINING_SET_FILE = TRAINING_SET_FILE,SIM_ANNEAL_INIT_SEQ_MUT_RADIUS = SIM_ANNEAL_INIT_SEQ_MUT_RADIUS):
    ...
    if use_25000_train_seqs == 'True':
        train_reps = load_model_25000(seed,ebd_path_dict,train_name_path,n_train_seqs,task_name)
    else:
        train_reps = load_model(ebd_path_dict,seq_name_path,train_seqs,task_name)
    return train_reps,train_qfunc,init_seqs,mu_muts_per_seq

#Generate representations

def top_model(train_reps,train_qfunc,TOP_MODEL_DO_SPARSE_REFIT,TOP_MODEL_ENSEMBLE_NMEMBERS = TOP_MODEL_ENSEMBLE_NMEMBERS,
...
def get_predict_data(task_name, ebd_path_dict, test_ebd_path, real_value_path, test_list_path_25000, seed, top_model, test_list_path, choose_test_data, test_data_separate):
    test_reps = []
    test_names = []
    d1 = {}
    d = {}
    if choose_test_data == '25000':
        path = ebd_path_dict[task_name]
        with open(test_list_path_25000) as f1:
...
def output_index_25000(d1,d,data_save,num,task_name,n_train_seqs):
    ...
    if data_save == 'True':
        ...
        np.save('/home/caiyi/low_n_output/for_recall/' + str(task_name) + '_' + str(n_train_seqs) + '_ALL_25000' + '/gfp_20210407_T12__KNN_double_brightonly_4263-' + str(num) + '.npy', all_data)
    r_model = 0
    rho_model = 0
    acc_model = 0
    auc_model = 0
...
def output_index(d1,d,data_save,test_data_separate,seed,task_name,n_train_seqs):
    print('len_d_is:',len(d),len(d1))
    #print(d.keys())
    d_real = {k: [] for k in d}
    ...
    if data_save == 'True' and test_data_separate == 'False':
        real = copy.copy(d_real['GFP_SimAnneal-'])
        predict = copy.copy(d_unirep_400['GFP_SimAnneal-'])
        print(len(real))
        print(len(predict))
        assert len(real) == len(predict)
        predict.extend(real)
        ...
        for i in range(len(real)):
            name_list.append(i)
        all_data = np.array([data_array,name_list])
        np.save('/home/caiyi/low_n_output/for_recall/' + str(task_name) + '_' + str(n_train_seqs) + '_ALL' +'/gfp_20210407_T12__KNN_double_brightonly_4263-' + str(seed) + '.npy', all_data)
...
def data_save(use_25000_train_seqs,d1,d,data_save,test_data_separate,seed,output_path,task_name,n_train_seqs,choose_test_data,train_num):
    if use_25000_train_seqs == 'True' and choose_test_data == '25000':
        r_model, rho_model, acc_model, auc_model, mse_model = output_index_25000(d1,d,data_save,seed,task_name,n_train_seqs)
        ...
        r_unirep, rho_unirep, acc_unirep, auc_unirep, mse_unirep, r_unirep_400, rho_unirep_400, acc_unirep_400, auc_unirep_400, mse_unirep_400 = output_index(d1,d,data_save,test_data_separate,seed,task_name,n_train_seqs)  
        ...
        r_unirep, rho_unirep, acc_unirep, auc_unirep, mse_unirep, r_unirep_400, rho_unirep_400, acc_unirep_400, auc_unirep_400, mse_unirep_400 = output_index(d1,d,data_save,test_data_separate,seed,task_name,n_train_seqs)  
        with open(output_path + '/Low_N_test/' + str(task_name) + '/' + 'output_' + task_name + '_' + str(n_train_seqs) + '_' + choose_test_data + '.txt', 'a') as f: 
            f.write(f'{task_name + train_num}\t{r_unirep_400}\t{rho_unirep_400}\t{acc_unirep_400}\t{auc_unirep_400}\t{r_unirep}\t{rho_unirep}\t{acc_unirep}\t{auc_unirep}\n')
