...
class babbler1900():
    ...
    def __init__(self,
        ...
        print("self._sample", self._sample)
        if sess is None:
    ...
    def get_all_hiddens(self, seq_list, sess, return_logits=False):
        ...
        #print('int_seq_list is :', int_seq_list)#, len(int_seq_list[0])
        ...
        # print('batch_is:', batch, batch.shape)
        # print('h2_ok')  
        # print(batch.shape)  
        if return_logits:
            hiddens, logits = sess.run([self._output, self._logits], feed_dict={
                self._minibatch_x_placeholder: batch,
                self._initial_state_placeholder: self._zero_state, 
                self._batch_size_placeholder: self._batch_size
                })
        else:
            hiddens = sess.run(self._output, feed_dict={
                self._minibatch_x_placeholder: batch,
                self._initial_state_placeholder: self._zero_state, 
                self._batch_size_placeholder: self._batch_size
                })
            # self._output: Tensor("rnn/transpose_1:0", shape=(?, ?, 1900), dtype=float32) 
            # self._minibatch_x_placeholder: Tensor("minibatch_x:0", shape=(?, ?), dtype=int32) 
            # self._initial_state_placeholder: (<tf.Tensor 'Placeholder:0' shape=(?, 1900) dtype=float32>, <tf.Tensor 'Placeholder_1:0' shape=(?, 1900) dtype=float32>) 
            # self._batch_size_placeholder: Tensor("batch_size:0", shape=(), dtype=int32)
        # print('h3_ok')    
